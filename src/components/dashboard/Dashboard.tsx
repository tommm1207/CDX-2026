...
export const Dashboard = ({ user, onNavigate, addToast, pendingApprovals = 0 }: DashboardProps) => {
  ...
  const menuActions = [
    ...
    { id: 'production-list', label: 'Lệnh sản xuất', icon: Layers, color: 'bg-indigo-600', description: 'Điều phối sản xuất và vật tư', adminOnly: true },
    { id: 'cost-report', label: 'Báo cáo chi phí', icon: FileText, color: 'bg-primary', description: 'Ghi chép chi tiêu dự án' },
    { id: 'inventory-report', label: 'Tồn kho', icon: BarChart3, color: 'bg-teal-600', description: 'Kiểm tra số lượng tồn kho' },
    { id: 'material-catalog', label: 'Danh mục vật tư', icon: Package, color: 'bg-cyan-600', description: 'Quản lý danh sách vật tư', adminOnly: true },
    { id: 'attendance', label: 'Chấm công', icon: CalendarCheck, color: 'bg-violet-600', description: 'Theo dõi chuyên cần hàng ngày' },
    { id: 'monthly-salary', label: 'Bảng lương', icon: Wallet, color: 'bg-emerald-600', description: 'Tổng hợp lương tháng', adminOnly: true },
    { id: 'pending-approvals', label: 'Phê duyệt', icon: ClipboardCheck, color: 'bg-amber-600', description: 'Duyệt phiếu chờ xử lý', adminOnly: true },
  ];
