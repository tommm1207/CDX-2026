...
export const CostReport = ({
  ...
  const filteredHistory = Object.values(groupedDataObj)
    ...
    .sort((a: any, b: any) => {
      ...
      return new Date(b.date).getTime() - new Date(a.date).getTime() || (b.cost_code || '').localeCompare(a.cost_code || '');
    });
    ...
    <div className="p-4 md:p-6 space-y-6 pb-24 overflow-x-hidden">
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-4 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 space-y-4">
                  <p className="text-[10px] font-black text-primary/40 uppercase tracking-widest text-center">Khoảng thời gian</p>
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Tìm kiếm báo cáo</label>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px] whitespace-nowrap">
            <thead>
              <tr className="bg-primary/5 text-primary border-b border-primary/10">
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-primary/5 w-16 text-center">STT</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-primary/5">Mã phí</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-primary/5">Ngày ghi</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-primary/5">Nhân sự</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-primary/5">Nội dung chính</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-primary/5 text-center">Hạng mục</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-primary/5 text-right">Tổng tiền</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest italic text-center">Thao tác</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr>
                   <td colSpan={8} className="px-6 py-12 text-center text-gray-400 font-bold italic">Đang tải báo cáo...</td>
                </tr>
              ) : filteredHistory.length === 0 ? (
                <tr>
                   <td colSpan={8} className="px-6 py-12 text-center text-gray-400 font-bold italic">Chưa có dữ liệu nào phù hợp với bộ lọc</td>
                </tr>
              ) : (
                filteredHistory.map((group: any, idx) => (
                  ...
                  >
                    <td className="px-6 py-4 text-center">
                      <span className="text-[10px] font-black text-gray-300 group-hover:text-primary/40 transition-colors">{(idx + 1).toString().padStart(2, '0')}</span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs font-bold text-gray-600 italic">{formatDate(group.date)}</td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-black text-gray-800 tracking-tight">{group.employee_name}</span>
                      </div>
                    </td>
                    ...
                    <td className={`px-6 py-4 text-sm font-black text-right ${group.total_amount >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                    ...
                    <td className="px-6 py-4 text-center">
                       <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                         ...
                         <button 
                           onClick={(e) => { e.stopPropagation(); /* TODO: Delete report logic */ }} 
      ...
      <div style={{ position: 'absolute', left: '-9999px', top: 0 }}>
        <div ref={reportRef} className="p-8 bg-white" style={{ width: '1400px' }}>
          ...
          <div className="mb-6">
            <h1 className="text-3xl font-black italic text-[#2D5A27] tracking-tighter mb-1">BÁO CÁO CHI PHÍ</h1>
          ...
          <table className="w-full text-left border-collapse whitespace-nowrap">
            <thead>
              <tr className="bg-primary text-white">
                ...
                <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider">Nội dung</th>
                <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider">Chứng từ</th>
            ...
            <tbody>
              {filteredHistory.map((group: any) => (
                <tr key={group.cost_code} className="border-b border-gray-100">
                  <td className="px-4 py-3.5 text-xs font-black text-primary uppercase">{group.cost_code}</td>
                  <td className="px-4 py-3.5 text-xs font-bold text-gray-600">{formatDate(group.date)}</td>
                  <td className="px-4 py-3.5 text-xs font-bold text-gray-800 truncate max-w-xs">{group.items[0]?.content}</td>
                  <td className="px-4 py-3.5 text-xs font-bold text-gray-400 uppercase tracking-tighter">{group.items.length} hạng mục</td>
                  <td className={`px-4 py-3.5 text-xs font-black text-right ${group.total_amount >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                    {formatCurrency(group.total_amount)}
                  </td>
                </tr>
              ))}
              <tr className="bg-gray-50 font-black">
                <td colSpan={4} className="px-4 py-4 text-xs text-right italic">TỔNG CHI PHÍ THỰC TẾ:</td>
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 flex justify-between items-end">
             ...
             <div className="flex items-center gap-2">
               ...
               <span className="text-[10px] font-bold text-gray-300 uppercase">Financial Integrity</span>
             </div>
          </div>
        </div>
      </div>
      ...
      >
        <div className="p-12 bg-white">
          ...
          <div className="mb-6">
            <h1 className="text-3xl font-black italic text-[#245D51] tracking-tighter mb-1 uppercase">BÁO CÁO TỔNG HỢP</h1>
          ...
          <div className="grid grid-cols-2 gap-8 mb-8 bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] text-gray-400 font-black uppercase tracking-[0.2em]">Cấu hình báo cáo</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[12px] text-gray-500 font-bold">Từ ngày:</p>
                  <p className="text-md font-black text-gray-900">{filters.fromDate ? formatDate(filters.fromDate) : '—'}</p>
                </div>
                <div>
                  <p className="text-[12px] text-gray-500 font-bold">Đến ngày:</p>
                  <p className="text-md font-black text-gray-900">{filters.toDate ? formatDate(filters.toDate) : '—'}</p>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] text-gray-400 font-black uppercase tracking-[0.2em]">Thông số hệ thống</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[12px] text-gray-500 font-bold">Tổng lô:</p>
                  <p className="text-md font-black text-primary italic">{filteredHistory.length} cụm phí</p>
                </div>
                <div>
          ...
          <table className="w-full text-left border-collapse min-w-[1000px] whitespace-nowrap">
            <thead>
              <tr className="bg-primary/5 text-primary border-b-2 border-primary/20">
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest italic border-r border-primary/5 w-16 text-center">STT</th>
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest italic border-r border-primary/5">Mã phí</th>
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest italic border-r border-primary/5">Ngày ghi</th>
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest italic border-r border-primary/5">Nhân sự</th>
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest italic border-r border-primary/5">Nội dung chính</th>
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest italic border-r border-primary/5 text-center">Hạng mục</th>
                <th className="px-6 py-4 text-[11px] font-black uppercase tracking-widest italic text-right">Tổng tiền</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredHistory.map((group: any, idx) => (
                <tr key={group.cost_code} className="hover:bg-primary/5 transition-all group">
                  ...
                  <td className="px-6 py-4 text-xs font-bold text-gray-600 italic">{formatDate(group.date)}</td>
                  <td className="px-6 py-4">
                    <span className="text-xs font-black text-gray-900 uppercase">{group.employee_name}</span>
                  </td>
                  <td className="px-6 py-4">
              ...
              <tr className="bg-primary/5 font-black border-t-2 border-primary/20">
                <td colSpan={6} className="px-6 py-4 text-xs uppercase tracking-widest italic text-right">Tổng phát sinh ròng:</td>
