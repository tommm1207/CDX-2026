    ...
    <div className="p-4 md:p-6 space-y-6 pb-24">
      ...
      >
        <div ref={reportRef} className="p-12 bg-white" style={{ width: '1400px' }}>
          ...
          <div className="mb-6">
            <h1 className="text-3xl font-black italic text-primary tracking-tighter mb-1 uppercase">BÁO CÁO CHI PHÍ</h1>
          ...
          <div className="grid grid-cols-2 gap-8 mb-8 bg-gray-50/50 p-6 rounded-3xl border border-gray-100">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em]">Thông tin chung</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-[11px] text-gray-500 font-bold">Tổng mục:</p>
                  <p className="text-sm font-black text-primary italic uppercase tracking-widest">{filteredCosts.length} bản ghi</p>
                </div>
                <div>
          ...
          <table className="w-full text-left border-collapse rounded-3xl overflow-hidden shadow-sm border border-gray-100">
            <thead>
              <tr className="bg-primary text-white">
                ...
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Loại chi</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Nội dung</th>
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {filteredCosts.map((item, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-primary/5'}>
                  ...
                  <td className="px-4 py-3 text-xs font-black text-primary tracking-tight">{item.cost_code}</td>
                  <td className="px-4 py-3 text-xs font-black text-gray-900 uppercase tracking-tight">{item.cost_type}</td>
                  <td className="px-4 py-3 text-xs font-bold text-gray-500 max-w-[250px] truncate">{item.content}</td>
                  <td className={`px-4 py-3 text-xs font-black text-right tabular-nums ${item.transaction_type === 'Thu' ? 'text-green-600' : 'text-rose-600'}`}>
                    {item.transaction_type === 'Thu' ? '+' : '-'}{formatCurrency(item.total_amount || 0)}
                  </td>
                </tr>
              ))}
              <tr className="bg-primary/5 font-black border-t-2 border-primary/20">
                <td colSpan={4} className="px-4 py-4 text-[11px] text-primary uppercase text-right italic tracking-[0.1em]">Tổng số dư phát sinh:</td>
                <td className="px-4 py-4 text-lg text-right tabular-nums text-primary underline decoration-double">
                  {formatCurrency(filteredCosts.reduce((sum, item) => sum + (item.transaction_type === 'Thu' ? (item.total_amount || 0) : -(item.total_amount || 0)), 0))}
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="mt-12 flex justify-between items-end border-t border-gray-100 pt-6">
            <div className="space-y-1">
              ...
              <p className="text-[9px] text-gray-300 font-bold uppercase tracking-widest">End of financial report • Accounting Integrity Verified</p>
            </div>
            <div className="text-right">
              ...
              <div className="text-[10px] text-gray-400 font-bold bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
                Verif-ID: <span className="text-primary font-black tracking-widest italic ml-1 underline">{new Date().getTime().toString(16).toUpperCase()}</span>
              </div>
            </div>
          </div>
        </div>
