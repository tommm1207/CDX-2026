        ...
        >
          ...
          <div className="flex justify-between items-start mb-10 pb-6 border-b-2 border-primary/20">
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-blue-100 italic">Inventory Flow Analysis</span>
            ...
            <div className="text-right">
              <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tighter mb-1">Báo Cáo Điều Chuyển Kho</h2>
              <p className="text-xs text-gray-500 font-bold italic">Thời gian xuất: {new Date().toLocaleString('vi-VN')}</p>
              <div className="mt-4 flex flex-col items-end gap-1">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest font-mono">Status: INTERNAL_FLOW</p>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm font-black text-primary italic uppercase tracking-widest">{statusFilter}</p>
                </div>
                <div>
          ...
          <table className="w-full text-left border-collapse rounded-3xl overflow-hidden shadow-sm border border-gray-100">
            <thead>
              <tr className="bg-primary text-white">
                ...
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Từ kho</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Đến kho</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic text-center">SL</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 text-sm">
              {(() => {
                const filteredList = slips.filter((item) => {
                  ...
                  if (filterStartDate && item.date < filterStartDate) match = false;
                  if (filterEndDate && item.date > filterEndDate) match = false;
                  if (filterWarehouseId && item.from_warehouse_id !== filterWarehouseId) match = false;
                  if (searchTerm) {
                    const s = searchTerm.toLowerCase();
                    const nameMatch = (item.materials?.name || '').toLowerCase().includes(s);
                    const codeMatch = (item.transfer_code || '').toLowerCase().includes(s);
                ...
                return filteredList.map((item, idx) => (
                  <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-primary/5'}>
                    ...
                    <td className="px-4 py-3 text-xs font-black text-primary tracking-tight">{item.transfer_code}</td>
                    <td className="px-4 py-3 text-xs font-black text-gray-900 uppercase tracking-tight">{item.materials?.name}</td>
                    <td className="px-4 py-3 text-xs font-bold text-gray-500">{item.from_warehouses?.name}</td>
                    <td className="px-4 py-3 text-xs font-bold text-gray-800">{item.to_warehouses?.name}</td>
                    <td className="px-4 py-3 text-xs font-black text-primary text-center tracking-tighter italic">{formatNumber(item.quantity)} {item.materials?.unit}</td>
                  </tr>
                ));
              })()}
              <tr className="bg-primary/10">
                <td colSpan={5} className="px-4 py-4 text-[11px] font-black text-primary uppercase text-right italic tracking-[0.1em]">Tổng số lượt điều chuyển:</td>
          ...
          <div className="mt-12 flex justify-between items-end border-t border-gray-100 pt-6">
            <div className="space-y-1">
              ...
              <p className="text-[9px] text-gray-300 font-bold uppercase tracking-widest">End of transaction report • Logistical Harmony</p>
            </div>
            <div className="text-right">
              <p className="text-[9px] font-black text-gray-300 uppercase tracking-[0.3em] mb-1">Transfer Protocol Verified</p>
              <div className="text-[10px] text-gray-400 font-bold bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
                System Hash: <span className="text-primary font-black tracking-widest italic ml-1 underline">SYNC_OK</span>
              </div>
            </div>
          </div>
        </div>
