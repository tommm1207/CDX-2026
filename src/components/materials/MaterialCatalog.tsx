        ...
        >
          ...
          <div className="flex justify-between items-start mb-10 pb-6 border-b-2 border-primary/20">
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100 italic">Inventory Assets Database</span>
            ...
            <div className="text-right">
              <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tighter mb-1">Danh Mục Vật Tư Hệ Thống</h2>
              <p className="text-xs text-gray-500 font-bold italic">Thời gian xuất: {new Date().toLocaleString('vi-VN')}</p>
              <div className="mt-4 flex flex-col items-end gap-1">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest font-mono">STATUS: MASTER_CATALOG</p>
          ...
          <table className="w-full text-left border-collapse rounded-3xl overflow-hidden shadow-sm border border-gray-100">
            <thead>
              <tr className="bg-gray-800 text-white">
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10 w-16 text-center">STT</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Mã vật tư</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Tên vật tư</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Nhóm vật tư</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic border-r border-white/10">Quy cách / Kích thước</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest italic text-right">Đơn vị</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100 text-xs">
              {filteredMaterials.map((item, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}>
                  ...
                  <td className="px-4 py-3 font-black text-primary font-mono tracking-tighter">#{item.code || item.id.slice(0, 6)}</td>
                  <td className="px-4 py-3 font-black text-gray-900 uppercase tracking-tight break-words max-w-[300px] whitespace-normal leading-relaxed">{item.name}</td>
                  <td className="px-4 py-3 text-gray-500 font-bold uppercase tracking-tight">{item.material_groups?.name || '—'}</td>
                  <td className="px-4 py-3 text-gray-400 italic font-medium">{item.specification || '—'}</td>
                  <td className="px-4 py-3 text-right font-black text-gray-900 uppercase italic tracking-widest bg-gray-100/30">{item.unit || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="mt-12 flex justify-between items-end border-t border-gray-100 pt-6">
            <div className="space-y-1">
              ...
              <p className="text-[9px] text-gray-300 font-bold uppercase tracking-widest">End of material catalog report • Integrated Data Asset Management</p>
            </div>
            <div className="text-right">
              <p className="text-[9px] font-black text-gray-300 uppercase tracking-[0.3em] mb-1">Database Integrity Chain</p>
              <div className="text-[10px] text-gray-400 font-bold bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
                Security Hash: <span className="text-primary font-black tracking-widest italic ml-1 underline">{new Date().getTime().toString(16).toUpperCase()}</span>
              </div>
            </div>
          </div>
        </div>
