        ...
        >
          ...
          <div className="flex justify-between items-start mb-10 pb-6 border-b-2 border-indigo-200">
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-1">
                <h1 className="text-3xl font-black text-indigo-600 tracking-tighter uppercase italic">CDX ERP SYSTEM</h1>
                ...
                <div className="flex items-center gap-3 mt-2">
                  <span className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-black uppercase tracking-widest border border-indigo-100 italic">Construction Diary Log</span>
                  <span className="w-1.5 h-1.5 bg-gray-200 rounded-full" />
                  <span className="text-[10px] text-gray-500 font-bold italic tracking-wide">Ref ID: DIARY_POOL_{new Date().getTime().toString(36).toUpperCase()}</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tighter mb-1">Tổng Hợp Nhật Ký Thi Công</h2>
              <p className="text-xs text-gray-500 font-bold italic">Thời gian xuất: {new Date().toLocaleString('vi-VN')}</p>
              <div className="mt-4 flex flex-col items-end gap-1">
                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest font-mono">STATUS: FIELD_AUDITED</p>
              ...
              {filteredDiaries.map((diary, idx) => (
                <tr key={idx} className={idx % 2 === 0 ? 'bg-white' : 'bg-indigo-50/10'}>
                  ...
                  <td className="px-4 py-3 text-center">
                    <span className="font-black text-indigo-600 font-mono tracking-tighter">#{diary.diary_code}</span>
                  </td>
                  <td className="px-4 py-3 font-black text-gray-900 uppercase tracking-tight">{(diary as any).warehouses?.name}</td>
                  <td className="px-4 py-3 text-gray-600 italic font-medium">{diary.labor_info || '—'}</td>
                  <td className="px-4 py-3 text-gray-800 leading-relaxed max-w-[400px] break-words">{diary.work_progress}</td>
                  <td className="px-4 py-3 text-center">
                    {diary.image_urls && diary.image_urls.length > 0 ? (
                      ...
                      <span className="text-gray-300 font-bold uppercase tracking-widest">NONE</span>
                    )}
                  </td>
                </tr>
              ))}
          ...
          <div className="mt-12 flex justify-between items-end border-t border-gray-100 pt-6">
            <div className="space-y-1">
              ...
              <p className="text-[9px] text-gray-300 font-bold uppercase tracking-widest">End of construction log report • Safety & Quality First</p>
            </div>
            <div className="text-right">
              <p className="text-[9px] font-black text-gray-300 uppercase tracking-[0.3em] mb-1">Operational Protocol Verified</p>
              <div className="text-[10px] text-gray-400 font-bold bg-gray-50 px-3 py-1 rounded-full border border-gray-100">
                Log Integrity: <span className="text-indigo-500 font-black tracking-widest italic ml-1 underline">{new Date().getTime().toString(16).toUpperCase()}</span>
              </div>
            </div>
          </div>
        </div>
