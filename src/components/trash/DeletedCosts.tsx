              ...
              {loading ? (
                ...
                costs.map((item) => (
                  <tr 
                    ...
                    onClick={() => handleRowClick(item.id, item.cost_code || item.materials?.name || item.content || 'Chi phí')}
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); handleRestoreClick(item.id, item.cost_code || item.materials?.name || item.content || 'Chi phí'); }}
        ...
        {showActionModal && selectedItem && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm" onClick={() => setShowActionModal(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center" onClick={e => e.stopPropagation()}>
              ...
              <p className="text-sm text-gray-500 mb-6">Bạn muốn làm gì với chứng từ chi phí <span className="font-bold text-gray-800">{selectedItem.name}</span>?</p>
