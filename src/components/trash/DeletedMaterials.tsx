...
import { Package, RefreshCw, Trash2, AlertTriangle, X, Check, Square, CheckSquare } from 'lucide-react';
...
export const DeletedMaterials = ({ onBack, addToast }: { 
  onBack: () => void,
  ...
  const fetchDeletedItems = async () => {
    ...
    try {
      ...
      const allDeleted = [
        ...(mats.data || []).map(m => ({ ...m, type: 'Vật tư', table: 'materials', displayName: m.name })),
        ...(groups.data || []).map(g => ({ ...g, type: 'Nhóm vật tư', table: 'material_groups', displayName: g.name }))
  ...
  const confirmRestore = async () => {
    ...
    try {
      const { error } = await supabase.from(selectedItem.table).update({ status: null }).eq('id', selectedItem.id);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      const usage = await checkUsage(selectedItem.table === 'materials' ? 'material' : 'group', selectedItem.id);
      if (usage.inUse) {
  ...
  const bulkDelete = async () => {
    try {
        ...
        if (addToast) addToast(`Đã xóa vĩnh viễn ${successCount} mục. ${failCount > 0 ? `Thất bại ${failCount} mục do đang được sử dụng.` : ''}`, 'success');
        fetchDeletedItems();
  ...
  const emptyTrash = async () => {
    try {
        ...
        if (addToast) addToast(`Đã dọn sạch ${successCount} mục. ${failCount > 0 ? `Bỏ qua ${failCount} mục đang được sử dụng.` : ''}`, 'info');
        fetchDeletedItems();
        ...
        <table className="w-full text-left border-collapse whitespace-nowrap">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-gray-400">Tên</th>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${item.type === 'Vật tư' ? 'bg-emerald-100 text-emerald-600' : 'bg-purple-100 text-purple-600'}`}>
                        {item.type}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-xs font-bold text-gray-800">{item.displayName}</div>
                      {item.code && <div className="text-[10px] text-gray-400 mt-0.5">Mã: {item.code}</div>}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex justify-end gap-2">
                        <button
                          onClick={(e) => { e.stopPropagation(); setSelectedItem({ id: item.id, table: item.table, name: item.displayName }); setShowRestoreModal(true); }}
                          className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-colors"
                        >
                          <RefreshCw size={14} />
                        </button>
                        <button
                          onClick={(e) => { e.stopPropagation(); setSelectedItem({ id: item.id, table: item.table, name: item.displayName }); setShowDeleteModal(true); }}
