...
import { Archive, RefreshCw, Trash2, AlertTriangle, X, Check, Square, CheckSquare } from 'lucide-react';
...
export const DeletedSlips = ({ onBack, addToast }: { 
  ...
  const confirmRestore = async () => {
    ...
    try {
      const { data: slip } = await supabase.from(selectedItem.table).select('*').eq('id', selectedItem.id).single();
      ...
      if (selectedItem.table === 'stock_out' || selectedItem.table === 'transfers') {
        ...
        if (Number(slip.quantity) > stockAtDate) {
          throw new Error(`Kho không đủ tồn (${stockAtDate}) để khôi phục phiếu xuất (${slip.quantity})`);
        }
      }
  ...
  const bulkRestore = async () => {
    try {
      ...
      if (addToast) addToast(`Thành công ${success}, thất bại ${fail}`, success > 0 ? 'success' : 'error');
      fetchDeletedSlips();
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 pb-44">
            ...
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="px-4 py-3 w-10">
                <button onClick={toggleSelectAll} className="p-1 text-gray-400 hover:text-primary transition-colors">
                  {selectedIds.size === slips.length && slips.length > 0 ? <CheckSquare size={20} className="text-primary" /> : <Square size={20} />}
                </button>
              </th>
              ...
              <th className="px-4 py-3 text-right text-[10px] font-bold uppercase tracking-wider text-gray-400">Thao tác</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {loading ? (
              <tr><td colSpan={7} className="px-4 py-12 text-center text-gray-400 italic">Đang tải...</td></tr>
            ) : slips.length === 0 ? (
              <tr><td colSpan={7} className="px-4 py-12 text-center text-gray-400 italic">Thùng rác trống</td></tr>
            ) : (
              slips.map((item) => {
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3" onClick={(e) => { e.stopPropagation(); toggleSelect(item.table, item.id); }}>
      ...
      <AnimatePresence>
        {showActionModal && selectedItem && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm" onClick={() => setShowActionModal(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-3xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center" onClick={e => e.stopPropagation()}>
              ...
              <p className="text-sm text-gray-500 mb-6">Đối tượng: <span className="font-bold text-gray-800">{selectedItem.name}</span></p>
              <div className="flex flex-col gap-3">
                <button onClick={() => { setShowActionModal(false); setShowRestoreModal(true); }} className="w-full py-4 px-4 rounded-xl font-bold text-green-700 bg-green-50 hover:bg-green-100 transition-colors flex items-center justify-center gap-3">
                  <RefreshCw size={20} /> Khôi phục dữ liệu
                </button>
                <button onClick={() => { setShowActionModal(false); setShowDeleteModal(true); }} className="w-full py-4 px-4 rounded-xl font-bold text-red-700 bg-red-50 hover:bg-red-100 transition-colors flex items-center justify-center gap-3">
        ...
        <ConfirmModal show={showRestoreModal} title="Khôi phục phiếu?" message={`Khôi phục ${selectedItem?.name} về trạng thái Chờ duyệt?`} onConfirm={confirmRestore} onCancel={() => setShowRestoreModal(false)} type="success" />
        <ConfirmModal show={showDeleteModal} title="Xóa vĩnh viễn?" message={`Hành động này KHÔNG thể hoàn tác. ${selectedItem?.name} sẽ bị xóa.`} onConfirm={confirmDelete} onCancel={() => setShowDeleteModal(false)} type="danger" />
        <ConfirmModal show={showBulkRestoreModal} title="Khôi phục các mục đã chọn?" message={`Bạn muốn khôi phục ${selectedIds.size} chứng từ?`} onConfirm={bulkRestore} onCancel={() => setShowBulkRestoreModal(false)} type="success" />
        <ConfirmModal show={showBulkDeleteModal} title="Xóa vĩnh viễn các mục đã chọn?" message={`Hành động này sẽ xóa ${selectedIds.size} chứng từ vĩnh viễn.`} onConfirm={bulkDelete} onCancel={() => setShowBulkDeleteModal(false)} type="danger" />
        <ConfirmModal show={showEmptyTrashModal} title="Dọn sạch thùng rác?" message="Xác nhận xóa VĨNH VIỄN toàn bộ chứng từ trong thùng rác?" onConfirm={emptyTrash} onCancel={() => setShowEmptyTrashModal(false)} type="danger" />
      </AnimatePresence>
    </div>
  );
};
